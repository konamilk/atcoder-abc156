...
fn main() {
    // let source = AutoSource::from("4 1 3");
    input!{
        // from source,
        n: u64,
        a: u64,
    ...
    let mut ans = Mint::new(2).pow(n) -1;

    let binom_a = binom_mod(n, a);
    let binom_b = binom_mod(n, b);

    println!("{}", ans - binom_a - binom_b);
}


use modint::ModInt1000000007 as Mint;

pub fn binom_mod(n: u64, mut k: u64) -> Mint {
    k = std::cmp::min(k, n-k);
    (0..n + 1)
        .rev()
        .zip(1..k + 1)
        .fold(Mint::new(1), |mut r, (n, d)| { r *= n; r /= d; r })
}

pub fn binom(n: u64, mut k: u64) -> u64 {
    if n < k {
        return 0
    }
    k = std::cmp::min(k, n-k);
